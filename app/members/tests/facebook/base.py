...
import requests
from ..exceptions.facebook_exception import FacebookNotFoundError, FacebookResponseError, FacebookAPIError
from ..logs.facebook_log import *
...
class FacebookUserAccess(object):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.id:
            logging.warning("FacebookUserAccess instantiated without id.")

    @staticmethod
    def get_app_access_token():
        """
        Calls API and returns application access token.
        Requires constants FACEBOOK_APP_ID and FACEBOOK_SECRET_CODE to be set.
        """
    ...
    def get_access_token(self):
        ...
        """
        url = FACEBOOK_HOST + "/%s/accounts/test-users" % FACEBOOK_APP_ID
    ...
    def get_access_code(self, redirect_uri=""):
        ...
        """
        url = FACEBOOK_HOST + "/oauth/client_code"
        ...
        if not access_code:
            logging.warning("No access_code in response.")
        return access_code
    ...
    def get_page_data(self):
        ...
        """
        url = FACEBOOK_HOST + "/%s/accounts" % self.id
        ...
        if not page_data:
            logging.warning("No data object in response.")
    ...
    def get_permissions(self):
        ...
        user's permissions:
            ...
            status
        """
        url = FACEBOOK_HOST + "/%s/permissions" % self.id
        params = {'access_token': self.app_access_token}
        response = requests.get(url, params=params)
