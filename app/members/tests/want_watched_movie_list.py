...
User = get_user_model()
print('want_watched_movie_list.py --> start')


class WantWatchedMovieListTest(APITestCase):
    URL = reverse('apis:members:want-movie', kwargs={'pk': 1})
    ...
    def get_want_watched_movie_list(self, user_want_movie=False, user_watched_movie=False):
        test_user_pk = 1
        page_user_pk = 3
        ...
        if user_want_movie:
            url = reverse('apis:members:want-movie', kwargs={'pk': page_user_pk})
            user_to_movie_cnt = UserToMovie.objects.filter(user=page_user_pk, user_want_movie=user_want_movie).count()
        if user_watched_movie:
            url = reverse('apis:members:watched-movie', kwargs={'pk': page_user_pk})
            user_to_movie_cnt = UserToMovie.objects.filter(user=page_user_pk, user_watched_movie=user_watched_movie).count()

        response = self.client.get(url, format='json')
        # print(f'response.data: {response.data}')
        # print(f'user_to_movie_cnt >> {user_to_movie_cnt}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], user_to_movie_cnt)

    def test_get_want_movie_list(self):
        self.get_want_watched_movie_list(user_want_movie=True)

    def test_get_watched_movie_list(self):
        self.get_want_watched_movie_list(user_watched_movie=True)

    def test_get_want_movie_list_with_not_exist_user_pk(self):
        test_user_pk = 1
        page_user_pk = User.objects.all().count() + 1
        # print(f'page_user_pk >> {page_user_pk}')
        token = Token.objects.get(user_id=test_user_pk)
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + token.key)
        url = reverse('apis:members:want-movie', kwargs={'pk': page_user_pk})
        response = self.client.get(url, format='json')
    ...
    def test_get_want_movie_list_with_invalid_token(self):
        page_user_pk = 3
